...
class ObjectMethodsMixin(object):
	...
	def test_getProductsOnClientsFromBackend(self):
		...
		for poc in self.expected.productOnClients:
	...
	def test_selectProductOnClient(self):
		...
		for productOnClient in self.expected.productOnClients:
			if (productOnClient.getClientId() == self.expected.client1.id):
				client1ProductOnClients.append(productOnClient)
		productOnClients = self.backend.productOnClient_getObjects(clientId = self.expected.client1.getId())
		for productOnClient in productOnClients:
			self.assertEqual(productOnClient.getClientId(), self.expected.client1.getId(), u"Found product %s on client %s but did not expect it." % (productOnClient.getProductId(), self.expected.client1.getId()))

	def test_selectProductOnClientById(self):
		productOnClients = self.backend.productOnClient_getObjects(clientId = self.expected.client1.getId(), productId = self.expected.product2.getId())
		self.assertEqual(len(productOnClients), 1, u"Expected one product on client, but found %s on backend." % (len(productOnClients)))
		self.assertEqual(productOnClients[0].getProductId(), self.expected.product2.getId(), u"Expected product %s on client but got %s" % (self.expected.product2.getId(), productOnClients[0].getProductId()))
		self.assertEqual(productOnClients[0].getClientId(), self.expected.client1.getId(), u"Found product %s on client %s but did not expect it." % (productOnClients[0].getProductId(), self.expected.client1.getId()))

	def test_updateProductsOnClients(self):
		self.expected.productOnClient2.setTargetConfiguration('forbidden')
		self.backend.productOnClient_updateObject(self.expected.productOnClient2)
		productOnClients = self.backend.productOnClient_getObjects(targetConfiguration = 'forbidden')
		self.assertIn(self.expected.productOnClient2, productOnClients, u"Expected product %s on client %s, but did not get it from backend." % (self.expected.productOnClient2.productId, self.expected.productOnClient2.clientId))
		
		self.expected.productOnClient2.setInstallationStatus('unknown')
		self.backend.productOnClient_updateObject(self.expected.productOnClient2)
		productOnClients = self.backend.productOnClient_getObjects(installationStatus = 'unknown')
		self.assertEqual(len(productOnClients), 1, u"Expected one product with statud 'unknown' on client %s, but did got %s from backend." % (self.expected.productOnClient2.clientId, len(productOnClients)))
		
	def test_deleteProductOnClient(self):
		self.backend.productOnClient_deleteObjects(self.expected.productOnClient2)
		productOnClients = self.backend.productOnClient_getObjects()

		self.assertNotIn(self.expected.productOnClient2, productOnClients, u"Expected product %s on client %s to be deleted, but found it on backend." % (self.expected.productOnClient2.productId, self.expected.productOnClient2.clientId))
		
	
	def test_insertFaultyPropertyState(self):
		try:
			pps0 = ProductPropertyState(
						productId  = self.expected.productProperty1.getProductId(),
						propertyId = self.expected.productProperty1.getPropertyId(),
	...
	def test_getProductPropertyStatesFromBackend(self):
		...
		for state in self.expected.productPropertyStates:
	...
	def test_deleteProductPropertyState(self):
		self.backend.productPropertyState_deleteObjects(self.expected.productPropertyState2)
		productPropertyStates = self.backend.productPropertyState_getObjects()
		self.assertNotIn(self.expected.productPropertyStates, productPropertyStates, u"Expected product property states %s to be deleted, but found it on backend." % self.expected.productPropertyStates)
		
	def test_insertProductPropertyState(self):
		self.backend.productPropertyState_deleteObjects(self.expected.productPropertyState2)
		
		self.backend.productPropertyState_insertObject(self.expected.productPropertyState2)
		productPropertyStates = self.backend.productPropertyState_getObjects()

		self.assertIn(self.expected.productPropertyState2, productPropertyStates, u"Expected product property state %s on backend, but did not find it." % self.expected.productPropertyState2)
