# -*- coding: utf-8 -*-
"""
opsi python library - Util - Rights
...
from OPSI.Util import findFiles, getfqdn, OPSI_GLOBAL_CONF
...
def setRights(path=u'/'):
    ...
    dirnames = [
        u'/tftpboot/linux', u'/home/opsiproducts', u'/var/log/opsi',
        u'/etc/opsi', u'/var/lib/opsi'
    ]
    if _isSUSEEnterpriseLinux():
        dirnames = [
            u'/var/lib/tftpboot/opsi', u'/var/log/opsi', u'/etc/opsi',
    ...
    if basedir.startswith('/opt/pcbin/install'):
        ...
        for dirname in dirnames:
            if dirname.startswith('/opt/pcbin/install'):
                ...
                break
        if not found:
            dirnames.append(u'/opt/pcbin/install')
...
def _isSUSEEnterpriseLinux():
    distro = Distribution()
    return 'suse linux enterprise server' in distro.distribution.lower()


def _getLocalFQDN():
    return forceHostId(getfqdn(conf=OPSI_GLOBAL_CONF))
