...
class OpsiPXEConfdBackend(ConfigDataBackend):
	...
	def _getResponsibleDepotId(self, clientId):
		configStates = self._getBackendInstance().configState_getObjects(configId = u'clientconfig.depot.id', objectId = clientId)
		if configStates and configStates[0].values:
			depotId = configStates[0].values[0]
		else:
			configs = self._getBackendInstance().config_getObjects(id = u'clientconfig.depot.id')
