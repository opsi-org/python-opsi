...
from opsi.opsiservice.model.type import to_string
...
def librsync_signature(filename: Path | str, base64_encoded: bool = True) -> str | bytes:
	...
	try:
		...
		raise RuntimeError(f"Failed to get librsync signature from {filename}: {to_string(sig_err)}") from sig_err


def librsync_delta_file(filename: Path | str, signature: bytes, deltafile: Path | str) -> None:
	"""
	...
	if not isinstance(filename, Path):
		filename = Path(filename)
	if not isinstance(deltafile, Path):
		deltafile = Path(deltafile)
	...
	try:
		...
		raise RuntimeError(f"Failed to write delta file {deltafile}: {to_string(sig_err)}") from sig_err


def librsync_patch_file(oldfile: Path | str, deltafile: Path | str, newfile: Path | str) -> None:
	"""
	Create the new file from old file and delta file.
	"""
	assert _librsync
	if not isinstance(oldfile, Path):
		oldfile = Path(oldfile)
	if not isinstance(deltafile, Path):
		deltafile = Path(deltafile)
	if not isinstance(newfile, Path):
	...
	try:
		...
		raise RuntimeError(f"Failed to patch file {oldfile}: {to_string(patch_err)}") from patch_err
