...
@dataclass
class AuditLogProductPropertyState:
	productId: str
	propertyId: str
	scope: str
	newValue: list[Any] | None = None
	...
	def __post_init__(self) -> None:
		self.productId = to_product_id(self.productId)
		self.propertyId = to_product_property_id(self.propertyId)
		self.scope = to_string(self.scope)
		if self.newValue is not None:
			self.newValue = to_list(self.newValue)
...
class AuditLogEventType(MappedStrEnum):
	...
	PRODUCT_PROPERTY_STATE_SET = "product_property_state.value.set"
	PRODUCT_PROPERTY_STATE_DELETED = "product_property_state.value.deleted"
...
class AuditLog(Entity):
	...
	def __init__(
		...
		productPropertyState: AuditLogProductPropertyState | dict[str, Any] | None = None,
	) -> None:
		...
		self.productPropertyState: AuditLogProductPropertyState | None = None
		...
		if productPropertyState is not None:
			self.setProductPropertyState(productPropertyState)
	...
	def getProductPropertyState(self) -> AuditLogProductPropertyState | None:
		return self.productPropertyState

	def setProductPropertyState(self, productPropertyState: AuditLogProductPropertyState | dict[str, Any]) -> None:
		if isinstance(productPropertyState, dict):
			self.productPropertyState = AuditLogProductPropertyState.from_dict(cast(dict[str, Any], productPropertyState))
			return
	...
	def to_dict(self) -> dict[str, Any]:
		...
		if self.productPropertyState is not None:
			object_dict["productPropertyState"] = asdict(self.productPropertyState)
		return object_dict
