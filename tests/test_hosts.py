...
def getClients():
	...
	client3 = OpsiClient(id="client3.test.invalid", description="Test client 3", notes="#############", inventoryNumber="XYZABC_1200292")
	...
	client5 = OpsiClient(id="client5.test.invalid", description="Test client 5", oneTimePassword="abe8327kjdsfda")
...
def test_verifyHosts(extendedConfigDataBackend):
	def nullifyUncomparableValues(hostDict):
		keys = ("lastSeen", "created", "inventoryNumber", "notes", "opsiHostKey", "isMasterDepot")
		for key in keys:
			hostDict[key] = None
...
def workbenchConfigServer(localHostFqdn, localWorkbenchPath, remoteWorkbenchPath):
	return OpsiConfigserver(id=localHostFqdn, workbenchLocalUrl=localWorkbenchPath, workbenchRemoteUrl=remoteWorkbenchPath)
