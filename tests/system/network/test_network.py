...
from opsi.system.network._network import _gethostbyaddr_with_timeout
...
def test_get_fqdn() -> None:
	...
	with (
		...
		mock.patch("opsi.system.network._network.run_command", return_value=Process),
		mock.patch("opsi.system.network._network.get_hostnames", lambda: ["hostname1", "hostname2", "hostname1.domain.test", "alias"]),
	):
...
def test_prepare_proxy_environment_file(tmp_path: Path) -> None:
	from opsi.system.environment import update_environment_from_config_files
