...
def testAddingHostToBackend(test_data_path):
	originalDhcpdFile = os.path.join(test_data_path, "backend", "dhcp_ki.conf")
	...
	with createTemporaryTestfile(originalDhcpdFile) as dhcpdFile:
		backend = DHCPDBackend(dhcpdConfigFile=dhcpdFile, reloadConfigCommand='/bin/echo "Reloading dhcpd.conf"')
...
def testUpdatingHostWhereAddressCantBeResolvedFails(dhcpBackendWithoutLookup):
	client = OpsiClient(id="unknown-client.test.invalid", hardwareAddress="00:99:88:77:77:21")
...
@pytest.fixture
def dhcpdBackend(dhcpdConf):  # noqa
	yield DHCPDBackend(dhcpdConfigFile=dhcpdConf._filename, reloadConfigCommand='/bin/echo "Reloading dhcpd.conf"')
...
def testUpdatingHostTriggersChangeInDHCPDConfiguration(dhcpBackendWithoutLookup):
	...
	def isElementInConfigFile(elem, caseInSensitive=False):
		...
		def lineConversion(line):
			return line.lower() if caseInSensitive else line
	...
	def getMissingInfo(term):
		return "Expected {term!r} to be in DHCPD config {file}".format(term=term, file=backend._dhcpdConfFile._filename)
