...
from opsi.opsi.service.server._opsiconfd import _opsiconfd_get_config
from opsi.process import ProcessError


@pytest.mark.linux
def test_get_opsiconfd_config() -> None:
	with patch("opsi.opsi.service.server._opsiconfd.OPSICONFD_GET_CONFIG_COMMAND", ["opsiconfd-command-not-found"]):
	...
	with patch("opsi.opsi.service.server._opsiconfd.run_command", return_value=Process):
