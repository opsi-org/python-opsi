...
def test_read_write_ca_cert_file(tmp_path: Path) -> None:
	...
	for idx in range(len(write_threads)):
		...
		write_error = write_threads[idx].err
		if write_error:
			print(write_error)
			traceback.print_tb(write_error.__traceback__)
		assert not write_error
		read_error = read_threads[idx].err
		if read_error:
			print(read_error)
			traceback.print_tb(read_error.__traceback__)
...
def test_client_certificate(tmp_path: Path, client_key_password: str) -> None:
	...
	with (
		...
		with (
			...
			pytest.raises((OpsiServiceClientCertificateError, OpsiServiceConnectionError)) as err,
		):
			client.get("/")
			# TODO: Why is this not always OpsiServiceClientCertificateError?
			if isinstance(err.value, OpsiServiceClientCertificateError):
	...
	with (
		...
		with ServiceClient(
			...
			cast(Any, client._session).cookies = None
			client.connect_messagebus()
...
def test_start_browser_for_sso_login_linux_fallback_uses_external_environment() -> None:
	...
	with environment(env_vars):
		with (
			...
			mock.patch("opsi.opsi.service.client._service_client.shutil.which", lambda cmd: "/usr/bin/gio" if cmd == "gio" else None),
			mock.patch("opsi.opsi.service.client._service_client.subprocess.Popen") as popen_mock,
			mock.patch(
			...
			webbrowser_open_mock.assert_called_once_with("https://example.test/auth/saml/login")
			assert popen_mock.call_count == 1
			assert popen_mock.call_args.args[0] == ("gio", "open", "https://example.test/auth/saml/login")
			assert popen_mock.call_args.kwargs["env"]["LD_LIBRARY_PATH"] == "/original/lib/path"
			assert "LD_LIBRARY_PATH_ORIG" not in popen_mock.call_args.kwargs["env"]
...
def test_start_browser_for_sso_login_linux_launcher_oserror_fallback() -> None:
	...
	with (
		mock.patch("opsi.opsi.service.client._service_client.sys.platform", "linux"),
		mock.patch(
			"opsi.opsi.service.client._service_client.shutil.which",
			lambda cmd: "/usr/bin/gio" if cmd in ("gio", "xdg-open") else None,
		),
		mock.patch(
			"opsi.opsi.service.client._service_client.subprocess.Popen",
			side_effect=[OSError("broken gio"), None],
		) as popen_mock,
		mock.patch(
			"opsi.opsi.service.client._service_client.webbrowser.open",
		...
		assert popen_mock.call_args_list[0].args[0] == ("gio", "open", "https://example.test/auth/saml/login")
		assert popen_mock.call_args_list[1].args[0] == ("xdg-open", "https://example.test/auth/saml/login")
	...
	with http_test_server(generate_cert=True, response_headers={"server": "opsiconfd 4.3.0.0 (uvicorn)"}) as server:
		with ServiceClient(f"https://127.0.0.1:{server.port}", verify="accept_all") as client:
			...
			with pytest.raises(OpsiServiceConnectionError) as exc_info:
				client.get("/", read_timeout="FAIL")  # ty: ignore[no-matching-overload]
...
def test_messagebus_reconnect() -> None:
	...
	with (
		...
		ServiceClient(f"https://127.0.0.1:{server.port}", verify="accept_all") as client,
	):
...
def test_get() -> None:
	...
	class ReqThread(Thread):
		def __init__(self, client: ServiceClient) -> None:
			super().__init__(daemon=True)
			self.client = client
			self.response: Response | tuple[int, str, dict, bytes] = (0, "", {}, b"")

		def run(self) -> None:
			self.client.get("/")
			self.response = self.client.get("test")
	...
	with http_test_server(
		...
		with ServiceClient(f"https://127.0.0.1:{server.port}", verify="accept_all") as client:
			...
			for thread in threads:
				...
				assert response.headers["x-1"] == "1"
				assert response.headers["x-2"] == "2"
...
def test_messagebus_ping() -> None:
	...
	with http_test_server(
		...
		with ServiceClient(f"https://127.0.0.1:{server.port}", verify="accept_all") as client:
			...
			cast(Any, client.messagebus).ping_timeout = None
			client.connect_messagebus()
			time.sleep(3)
...
def test_jsonrpc_interface(tmp_path: Path) -> None:
	...
	with http_test_server(generate_cert=True, log_file=log_file, response_headers={"server": "opsiconfd 4.2.0.285 (uvicorn)"}) as server:
		with ServiceClient(f"https://127.0.0.1:{server.port}", verify="accept_all", jsonrpc_create_methods=True) as client:
			...
			dynamic_client.test_method(1, 2, x=3, y=4)
			dynamic_client.test_method(1, x="y")
			dynamic_client.test_method(1, x="y")

			dynamic_client.boot_getConfig()
			dynamic_client.boot_getConfig(architecture="x64", firmware_type="UEFI", ip_version=4)
			dynamic_client.boot_getConfig(firmware_type="UEFI", ip_version=4, architecture="x64")
			dynamic_client.boot_getConfig("x64", ip_version=4, firmware_type="UEFI")
			dynamic_client.boot_getConfig(firmware_type="UEFI")
			...
			dynamic_test_obj = cast(Any, test_obj)
			dynamic_test_obj.backend_getInterface()
			dynamic_test_obj.test_method(1, x="y")
			dynamic_test_obj.backend_exit()
			reqs = [json.loads(req) for req in log_file.read_text(encoding="utf-8").strip().split("\n")]
...
def test_backend_manager_and_get_service_client(tmp_path: Path) -> None:
	...
	with (
		...
		with catch_warnings():
			...
			cast(Any, backend).test_method(arg1=1, arg2=2)
