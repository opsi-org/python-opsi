...
from opsi.opsi.messagebus._process import process_process_message, processes, stop_running_processes
...
async def test_execute_command(close_stdin: bool) -> None:
	...
	await process_process_message(process_start_request, send_message=message_sender.send_message)
	...
	if not is_windows():
		...
		await process_process_message(process_data_write_message, send_message=message_sender.send_message)
		...
		if close_stdin:
			...
			await process_process_message(process_stop_request_message, send_message=message_sender.send_message)
