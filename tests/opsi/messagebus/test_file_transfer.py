...
async def test_file_upload(tmp_path: Path) -> None:
	...
	await process_file_transfer_message(file_upload_request, send_message=message_sender.send_message)
	...
	for overwrite in (True, False, True):
		...
		with test_file.open("rb") as file:
			...
			while data := file.read(chunk_size):
				...
				await process_file_transfer_message(file_chunk_message, send_message=message_sender.send_message)
...
async def test_upload_chunk_timeout(tmp_path: Path) -> None:
	...
	with patch("opsi.opsi.messagebus._file_transfer.FileUpload.chunk_timeout", chunk_timeout):
		file_upload_request = FileUploadRequestMessage(
			sender=sender,
			channel=channel,
...
async def test_stop_running_transfers(tmp_path: Path) -> None:
	...
	with patch("opsi.opsi.messagebus._file_transfer.FileUpload.default_destination", tmp_path):
		file_upload_request = FileUploadRequestMessage(
			sender=sender,
			channel=channel,
			content_type="application/octet-stream",
...
async def test_file_download(tmp_path: Path) -> None:
	...
	await process_file_transfer_message(file_download_request, send_message=message_sender.send_message)
...
async def test_file_download_follow(tmp_path: Path) -> None:
	...
	await process_file_transfer_message(file_follow_request, send_message=message_sender.send_message, sender=sender, back_channel=channel)
