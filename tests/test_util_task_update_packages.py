...
def test_download_with_range_retry(
	tmp_path, package_updater_class
	...
	server_package_file = server_dir / "hwaudit_4.2.0.0-1.opsi"
	server_package_file.write_bytes(b"0123456789")
	local_package_file = local_dir / server_package_file.name

	def write_repo_conf(base_url):
		test_repo_conf.write_text(
			data=(
				f"[repository_test]\nactive = true\nbaseUrl = {base_url}\ndirs = /\n"
				"autoInstall = true\nusername = user\npassword = pass\n"
	...
	with http_test_server(serve_directory=server_dir, log_file=str(server_log)) as server:
		...
		with package_updater.makeSession(package["repository"]) as session:
			server.set_option("send_max_bytes", 5)

			with pytest.raises(
				RuntimeError, match="Failed to complete download, only 5 of 10 bytes transferred, server does not support range requests"
			):
				package_updater.downloadPackage(package, session)

			server_log.unlink()

			server.set_option("response_headers", {"accept-ranges": "bytes"})

			package_updater.downloadPackage(package, session)
			requests = [json.loads(req) for req in server_log.read_text(encoding="utf-8").rstrip().split("\n")]
			assert len(requests) == 2
			assert requests[1]["headers"]["Range"] == "bytes=5-"

			print(local_package_file)
			assert local_package_file.exists()
			assert server_package_file.read_bytes() == local_package_file.read_bytes()
