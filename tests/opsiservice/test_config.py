...
from opsi.opsiservice.config import DEFAULT_OPSICONFD_USER, OPSICONFD_CONF, OpsiConfig, get_opsiconfd_user
...
def test_fill_from_legacy_config_depotserver(tmp_path: Path) -> None:
	...
	with (
		patch("opsi.opsiservice.config.DISPATCH_CONF", str(dispatch_conf)),
		patch("opsi.opsiservice.config.JSONRPC_CONF", str(jsonrpc_conf)),
	):
...
def test_fill_from_legacy_config_configserver(tmp_path: Path) -> None:
	...
	with (
		patch("opsi.opsiservice.config.DISPATCH_CONF", str(dispatch_conf)),
		patch("opsi.opsiservice.config.MYSQL_CONF", str(mysql_conf)),
		patch("opsi.opsiservice.config.GLOBAL_CONF", str(global_conf)),
	):
...
def test_run_as_user_value_from_config_file(tmp_path: Path) -> None:
	...
	with patch("opsi.opsiservice.config.OPSICONFD_CONF", str(confd_conf)):
		get_opsiconfd_user.cache_clear()
