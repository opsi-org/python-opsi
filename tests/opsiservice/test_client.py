...
from opsi.opsiservice.client._common import (
	MIN_VERSION_GZIP,
	MIN_VERSION_LZ4,
...
from opsi.opsiservice.model.object import OpsiClient
...
def test_get_opsi_ca_certs_state(tmp_path: Path) -> None:
	...
	with mock.patch("opsi.crypt.ssl._common.CertificateBuilder", MockCertificateBuilder):
...
def test_client_certificate(tmp_path: Path, client_key_password: str) -> None:
	...
	with (
		...
		proxy_port = 18182
		with run_proxy(proxy_port) as proxy_server, mock.patch("opsi.opsiservice.client._common.ServiceClient.no_proxy_addresses", []):
			with ServiceClient(
				f"https://127.0.0.1:{server.port}",
				verify=ServiceVerificationFlags.STRICT_CHECK,
				ca_cert_file=ca_cert_file,
...
def test_sso(tmp_path: Path, sso_success: bool) -> None:
	...
	with (
		...
		mock.patch("opsi.opsiservice.client._common.webbrowser.open", mock_webbrowser_open),
	):
...
def test_proxy(tmp_path: Path) -> None:
	...
	with (
		...
		mock.patch("opsi.opsiservice.client._common.ServiceClient.no_proxy_addresses", no_proxy_addresses),
	):
		# Proxy must not be used (no_proxy_addresses)
		with mock.patch(
			"opsi.opsiservice.client._common.ServiceClient.no_proxy_addresses", ["::1", "127.0.0.1", "ip6-localhost", "localhost", local_ip]
		):
			with ServiceClient(
...
def test_download(tmp_path: Path) -> None:
	...
	with http_test_server(generate_cert=True, serve_directory=remote_dir) as server:
		with ServiceClient(f"https://127.0.0.1:{server.port}", verify="accept_all") as client:
			with mock.patch("opsi.opsiservice.client._common.ServiceClient.webdav_content", mocked_webdav_content):
...
def test_messagebus_ping() -> None:
	...
	with http_test_server(
		...
		with mock.patch("opsi.opsiservice.client._common.Messagebus._on_pong", _on_pong):
			with ServiceClient(f"https://127.0.0.1:{server.port}", verify="accept_all") as client:
				client.messagebus.ping_interval = 1
				client.messagebus.ping_timeout = None
				client.connect_messagebus()
	...
	with use_logging_config(stderr_level=3):
		with http_test_server(generate_cert=True, log_file=log_file, request_callback=request_callback) as server:
			with opsi_config(
				...
				with (
					mock.patch("opsi.opsiservice.client._common.OPSI_CA_CERT_FILE", server.ca_cert),
					mock.patch("opsi.opsiservice.client._common.get_opsi_config", lambda: opsi_conf),
				):
					with catch_warnings():
						simplefilter("ignore")
					...
					with mock.patch(
						"opsi.opsiservice.client._common.get_opsiconfd_config",
						lambda *args, **kwargs: {
...
def test_messagebus_jsonrpc() -> None:
	...
	with http_test_server(
		...
		with ServiceClient(f"https://127.0.0.1:{server.port}", verify="accept_all") as client:
			...
			with mock.patch("opsi.opsiservice.client._common.RPC_TIMEOUTS", {"test": 1}):
				with pytest.raises(OpsiServiceTimeoutError):
...
def test_server_date_update() -> None:
	...
	with (
		mock.patch("opsi.opsiservice.client._common.set_system_datetime", mock_set_system_datetime),
...
def test_permission_error_ca_cert_file_lock() -> None:
	from opsi.file.lock._windows import _lock_file, _unlock_file
