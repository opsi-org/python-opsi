...
def test_temp_dir_retry_on_io_error() -> None:
	...
	with (
		patch("opsi.file.temp._common.TempDir._create_attempt", side_effect=side_effect_create, autospec=True),
		patch("opsi.file.temp._common.TempDir._delete_attempt", side_effect=side_effect_delete, autospec=True),
	):
		with TempDir() as temp_dir:
			assert temp_dir.exists()
...
def test_temp_file_retry_on_io_error() -> None:
	...
	with (
		patch("opsi.file.temp._common.TempFile._create_attempt", side_effect=side_effect_create, autospec=True),
		patch("opsi.file.temp._common.TempFile._delete_attempt", side_effect=side_effect_delete, autospec=True),
	):
		with TempFile() as temp_file:
			assert temp_file.path.exists()
