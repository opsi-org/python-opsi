# This file is part of the desktop management solution OPSI http://www.opsi.org
# Copyright (c) 2026-2026 uib GmbH <info@uib.de>
# This code is owned by the uib GmbH, Mainz, Germany (uib.de). All rights reserved.
# License: AGPL-3.0-only

import shutil
from unittest.mock import patch

from opsi.file.temp import TempDir, TempFile, create_temp_dir, create_temp_file
from opsi.system.info import is_linux


def test_temp_dir():
	with TempDir() as temp_dir:
		assert temp_dir.exists()
		if is_linux():
			assert str(temp_dir).startswith("/tmp/opsi_temp_")
			assert temp_dir.stat().st_mode & 0o777 == 0o700
		(temp_dir / "sub1" / "sub2").mkdir(parents=True)
		(temp_dir / "sub1" / "sub2" / "file.txt").write_text("Temp file content")
		(temp_dir / "sub1" / "file2.txt").write_text("Another temp file")

	assert not temp_dir.exists()


def test_create_temp_dir():
	path = create_temp_dir()
	assert path.is_dir()
	shutil.rmtree(path)


def test_temp_dir_retry_on_io_error() -> None:
	create_attempt = 0
	delete_attempt = 0
	orig_create = TempDir._create
	orig_delete = TempDir._delete

	def side_effect_create(self) -> None:
		nonlocal create_attempt
		create_attempt += 1
		if create_attempt < 2:
			raise OSError("Create error")
		return orig_create(self)

	def side_effect_delete(self) -> None:
		nonlocal delete_attempt
		delete_attempt += 1
		if delete_attempt < 2:
			raise OSError("Delete error")
		return orig_delete(self)

	with (
		patch("opsi.file.temp._common.TempDir._create", side_effect=side_effect_create, autospec=True),
		patch("opsi.file.temp._common.TempDir._delete", side_effect=side_effect_delete, autospec=True),
	):
		with TempDir() as temp_dir:
			assert temp_dir.exists()

	assert create_attempt == 2
	assert delete_attempt == 2


def test_temp_file():
	with TempFile() as temp_file:
		assert temp_file.exists()
		if is_linux():
			assert str(temp_file).startswith("/tmp/opsi_temp_")
			assert temp_file.stat().st_mode & 0o777 == 0o600
		temp_file.write_text("OPSI")
		assert temp_file.read_text() == "OPSI"
	assert not temp_file.exists()


def test_temp_file_retry_on_io_error() -> None:
	create_attempt = 0
	delete_attempt = 0
	orig_create = TempFile._create
	orig_delete = TempFile._delete

	def side_effect_create(self) -> None:
		nonlocal create_attempt
		create_attempt += 1
		if create_attempt < 2:
			raise OSError("Create error")
		return orig_create(self)

	def side_effect_delete(self) -> None:
		nonlocal delete_attempt
		delete_attempt += 1
		if delete_attempt < 2:
			raise OSError("Delete error")
		return orig_delete(self)

	with (
		patch("opsi.file.temp._common.TempFile._create", side_effect=side_effect_create, autospec=True),
		patch("opsi.file.temp._common.TempFile._delete", side_effect=side_effect_delete, autospec=True),
	):
		with TempFile() as temp_file:
			assert temp_file.exists()

	assert create_attempt == 2
	assert delete_attempt == 2


def test_create_temp_file():
	path = create_temp_file()
	assert path.is_file()
	path.unlink()
